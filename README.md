...
<style>
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .wallet-required-banner {
    background: linear-gradient(135deg, rgba(192,57,43,0.12), rgba(120,20,10,0.1));
  ...
  .wallet-required-banner.hidden { display: none; }
...
<div class="game-container">
  ...
  <div class="arena-col">
    ...
    <div class="action-area panel" id="action-area">
      ...
      <button class="btn-one-strike hidden" id="btn-venom-fight" onclick="playerAction('venom')" style="border-color:#7c3aed;color:#a78bfa;background:linear-gradient(135deg,#1a0830,#3a1060);">
        🧪 VENOM OF SPITE
        <div style="font-size:0.75rem;opacity:0.7;margin-top:0.2rem;font-family:'IM Fell English',serif;font-style:italic;">Poisons enemy · 10% max HP per turn · One use per fight</div>
      </button>

      <button class="btn-one-strike hidden" id="btn-veil-fight" onclick="playerAction('veil')" style="border-color:#2980b9;color:#60a5fa;background:linear-gradient(135deg,#0a1828,#103050);">
        🌫️ VEIL OF SHADOWS
        <div style="font-size:0.75rem;opacity:0.7;margin-top:0.2rem;font-family:'IM Fell English',serif;font-style:italic;">25% enemy miss chance · Active all fight · One use per fight</div>
      </button>
...
function startFight() {
  if (!shopState.walletAddress) {
    addLog(`👛 <span style="color:var(--red2)">You must save a wallet address first! Go to XP Shop → Name tab.</span>`, 'system');
    switchShopTab('name');
    renderWalletDisplay();
    return;
  }
...
function playerAction(type) {
  ...
  // Venom of Spite — activate mid-fight (no turn cost, just apply)
  if (type === 'venom') {
    if (!potionState.poison.owned || potionState.poison.activeThisFight) return;
    potionState.poison.activeThisFight = true;
    potionState.poison.owned = false;
    addLog(`🧪 <span style="color:#a78bfa">Venom of Spite applied! ${currentEnemy.name} takes 10% of their max HP as poison each turn.</span>`, 'system');
    updatePotionFightButtons();
    saveGame();
    return;
  }

  // Veil of Shadows — activate mid-fight (no turn cost)
  if (type === 'veil') {
    if (!potionState.miss.owned || potionState.miss.activeThisFight) return;
    potionState.miss.activeThisFight = true;
    potionState.miss.owned = false;
    addLog(`🌫️ <span style="color:#60a5fa">Veil of Shadows woven! ${currentEnemy.name} now has a 25% chance to miss each attack.</span>`, 'system');
    updatePotionFightButtons();
    saveGame();
    return;
  }
...
function playerWins() {
  ...
  document.getElementById('btn-venom-fight').classList.add('hidden');
  document.getElementById('btn-veil-fight').classList.add('hidden');
...
function updatePotionFightButtons() {
  const venomBtn = document.getElementById('btn-venom-fight');
  const veilBtn  = document.getElementById('btn-veil-fight');
  if (!venomBtn || !veilBtn) return;

  // Venom: show if owned (not yet used) OR already active this fight
  if (potionState.poison.owned || potionState.poison.activeThisFight) {
    venomBtn.classList.remove('hidden');
    venomBtn.disabled = potionState.poison.activeThisFight; // disabled once active (already used)
    if (potionState.poison.activeThisFight) {
      venomBtn.style.opacity = '0.5';
      venomBtn.querySelector('div').textContent = 'Poison already active this fight';
    } else {
      venomBtn.style.opacity = '';
      venomBtn.querySelector('div').textContent = 'Poisons enemy · 10% max HP per turn · One use per fight';
    }
  } else {
    venomBtn.classList.add('hidden');
  }

  // Veil: show if owned OR already active this fight
  if (potionState.miss.owned || potionState.miss.activeThisFight) {
    veilBtn.classList.remove('hidden');
    veilBtn.disabled = potionState.miss.activeThisFight;
    if (potionState.miss.activeThisFight) {
      veilBtn.style.opacity = '0.5';
      veilBtn.querySelector('div').textContent = 'Veil already active this fight';
    } else {
      veilBtn.style.opacity = '';
      veilBtn.querySelector('div').textContent = '25% enemy miss chance · Active all fight · One use per fight';
    }
  } else {
    veilBtn.classList.add('hidden');
  }
}
